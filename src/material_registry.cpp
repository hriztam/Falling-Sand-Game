...
const MaterialDef* MaterialRegistry::get(MaterialId id) const
{
    ...
    if (m_defs[id].name.empty()) return nullptr; // gap slot
    return &m_defs[id];
}
...
// ---------------------------------------------------------------------------
// buildDefaults — registers Empty, Sand, Water, Wall.
...
MaterialRegistry MaterialRegistry::buildDefaults()
{
    ...
    // --- Water (MAT_WATER = 2) -------------------------------------------
    {
        ...
        d.traits        = Trait::Movable | Trait::AffectedByGravity
                        | Trait::Displaceable | Trait::LiquidLike;
        d.density       = 1.0f;
        d.spreadFactor  = 6;   // replaces old WATER_FLOW = 6
        d.shadeMin      = 110;
        d.shadeMax      = 140;
